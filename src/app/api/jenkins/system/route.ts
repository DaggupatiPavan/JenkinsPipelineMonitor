...
export async function GET(request: NextRequest) {
  try {
    ...
    const computerInfo = await jenkinsService.getComputerInfo()
    
    return NextResponse.json(computerInfo)
  } catch (error) {
    console.error('Error fetching Jenkins system info:', error)
    return NextResponse.json(
      { error: 'Failed to fetch Jenkins system info' },
      { status: 500 }
    )
  }
}